...
Textbox::Textbox(int l_visibleLines, int l_charSize,
                 int l_width, sf::Vector2f l_screenPos){
    Setup(l_visibleLines, l_charSize, l_width, l_screenPos);
}

Textbox::~Textbox(){ Clear(); }

void Textbox::Setup(int l_visibleLines, int l_charSize,
                    int l_width, sf::Vector2f l_screenPos){
    m_numVisible = l_visibleLines;

    sf::Vector2f l_offset(2.0f, 2.0f);

//    m_font.loadFromFile("D:/dev/Fonts/arial.ttf");
    ...
    m_backdrop.setSize(sf::Vector2f(
            l_width, (l_visibleLines * (l_charSize * 1.2f))));
    m_backdrop.setFillColor(sf::Color(0,100,100,100));
    m_backdrop.setPosition(l_screenPos);
}

void Textbox::Add(std::string l_message){
    m_messages.push_back(l_message);
    if (m_messages.size() < m_numVisible+1){ return; }
